/* ============================================================
   ...
   5. crossfade le fond vidéo en alternant deux champs A/B (chacun une
      vidéo de dégradé en boucle continue) à chaque changement d'onglet ;
...
(function () {
  ...
  var AUTOPLAY_MS = 2400;   // intervalle entre deux onglets
  var FIRST_DELAY_MS = 400; // le carrousel démarre presque tout de suite
  var MIN_DURATION = 170;   // ms, saut entre deux onglets voisins
  var MAX_EXTRA = 150;      // ms ajoutées au maximum pour un saut d'un bord à l'autre
  var PX_PER_MS = 0.15;     // pente mesurée sur la référence, resserrée à la demande
  ...
  var bgVideos = Array.prototype.slice.call(root.querySelectorAll('.hero-tabs__bgvideo'));

  if (!track || !pill || tabs.length === 0 || tabs.length !== slides.length) return;

  // L'attribut `autoplay` seul ne suffit pas toujours (onglet créé en
  // arrière-plan, politique du navigateur) : on force la lecture, en
  // ignorant silencieusement un rejet de la promesse.
  ...
  // Chaque champ porte une vidéo fixe qui tourne en boucle en continu. Un
  // changement d'onglet se contente de cycler l'opacité vers le champ
  // suivant — jamais de recharger la vidéo — ce qui fait crossfader la
  // liste des vidéos les unes vers les autres, dans l'ordre, en boucle.
  function swapBackground(immediate) {
    if (fields.length < 2) return;
    if (immediate) {
      fields[activeField].classList.add('is-active');
      return;
    }
    var target = (activeField + 1) % fields.length;
    fields[target].classList.add('is-active');
